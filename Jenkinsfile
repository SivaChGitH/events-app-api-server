...
//        e.g https://github.com/SivaChGitH/events-app-api-server.git.git might become https://github.com/MyName/external.git
// Look for the following variables in this file and replace them:
//      sivachJenkin  //id of your global docker credentials in Jenkins https://www.google.com/search?q=add+docker+credentials+Jenkins&oq=add+docker+credentials+Jenkins
//      https://github.com/SivaChGitH/events-app-api-server.git
//      sivachdhub
//      api-server-image
//      cnd-events-cluster 
//      us-east-1  
...
pipeline {
    agent any 
   environment {
        registryCredential = 'sivachJenkin'
        imageName = 'sivachdhub/api-server-image'
        dockerImage = ''
        }
    stages {
        stage('Run the tests') {
            ...
            steps {
                ...
                git branch: 'main',
                    url: 'https://github.com/SivaChGitH/events-app-api-server.git'
        ...
        stage('Building image') {
         ...
         stage('update k8s') {
            ...
            steps {
                echo 'Set the image'
                     sh "kubectl -n user20 set image deployment/demo-api demo-api=${env.imageName}:${env.BUILD_NUMBER} --kubeconfig=${WORKSPACE}/.kube/config"
            }
        }     
